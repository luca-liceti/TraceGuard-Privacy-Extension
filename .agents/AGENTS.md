...
- Never credit an AI tool or assistant in this repository, in any commit message, code, comment, doc, changelog, or PR text. That forbids a `Generated with ...` trailer, a `Co-Authored-By` line naming an assistant, a robot emoji used as a credit marker, and the name of the tool anywhere in a commit message. There are no exceptions: the credit is not wanted, and it is not an accurate description of work that a person reviewed and owns. The only file allowed to contain an assistant name is `traceguard-extension/scripts/check-commit-messages.mjs`, which needs the names in order to detect them.
- `.github/workflows/commit-messages.yml` runs that checker over the commits a push or a pull request adds, on every branch, so a message carrying credit fails CI instead of relying on someone remembering. It matches credit lines rather than vocabulary, so naming a tool as a product decision (Gemini Nano, record 0008) still passes.
