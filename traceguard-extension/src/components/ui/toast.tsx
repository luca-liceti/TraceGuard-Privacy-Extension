...
function ToastContent({ className, ...props }: ToastPrimitive.Content.Props) {
  return (
    <ToastPrimitive.Content
      ...
      className={cn(
        "flex h-full items-center gap-3 overflow-hidden p-4 transition-opacity transition-duration-[250ms] transition-timing-function-[cubic-bezier(0.22,1,0.36,1)] data-[behind]:opacity-0 data-[expanded]:opacity-100",
        className
      )}
      {...props}
    />
